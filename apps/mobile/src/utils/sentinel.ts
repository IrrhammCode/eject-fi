...
import { getProtocolActivity, detectWhaleExits, getNetworkHealth, ProtocolActivity } from './helius';

// --- Configuration ---
const SOLANA_RPC = process.env.EXPO_PUBLIC_SOLANA_RPC || 'https://api.devnet.solana.com';
...
// Known DeFi protocol addresses on devnet (for utilization monitoring)
...
async function fetchProtocolUtilization(protocolAddress: PublicKey): Promise<number> {
  try {
    ...
    // Heuristic: map balance range to utilization percentage
    if (solBalance < 1) return 95 + Math.random() * 4;
    if (solBalance < 10) return 85 + Math.random() * 10;
    if (solBalance < 100) return 60 + Math.random() * 20;
    return 30 + Math.random() * 30;
  } catch (error) {
    console.warn('[Sentinel] Utilization fetch failed:', error);
...
export async function checkProtocolHealth(protocol: string): Promise<ProtocolHealth> {
  ...
  const [priceData, utilization, withdrawalVelocity, activity, whaleData, networkHealth] = await Promise.all([
    fetchPythPrice(),
    fetchProtocolUtilization(KAMINO_MAIN_MARKET),
    estimateWithdrawalVelocity(KAMINO_MAIN_MARKET),
  ...
  return {
    ...
    socialSentiment,
    activity,
    whaleExits: { amountUSD: whaleData.amountUSD, count: whaleData.count },
