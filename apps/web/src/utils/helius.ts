...
export async function getProtocolActivity(protocolName: string): Promise<ProtocolActivity> {
  ...
  try {
    ...
    console.warn('[Helius] Live fetch failed, using network average:');
    // Fallback based on Solana network averages instead of total randomness
    const baseTpm = 25; 
    return buildActivity(baseTpm, 0, 'fallback', 0, 0.4);
  }
}
...
export async function detectWhaleExits(): Promise<{
  ...
  try {
    ...
    // We don't hardcode price here anymore, we pass lamports to sentinel for conversion
    return { amountUSD: solAmount, count: whaleCount, dataSource: 'live' };
  } catch (error) {
    return { amountUSD: 0, count: 0, dataSource: 'fallback' };
  }
}
